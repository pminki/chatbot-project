...
export const ChatPage: React.FC<ChatPageProps> = ({ userId = 'guest-user' }) => {
  ...
  /**
   * [채팅창 렌더링 함수]
   * 전체화면 상태와 일반 상태에서 공통으로 사용되는 UI입니다.
   */
  const renderChatContent = () => (
    <div
      id="ai-chatbot-window"
      className={`flex flex-col bg-slate-50 shadow-2xl overflow-hidden border border-slate-200 font-sans relative transition-all duration-300 ease-in-out ${
        isExpanded
      ...
      <footer className="px-5 py-5 bg-white border-t border-slate-100 shadow-[0_-4px_12px_rgba(0,0,0,0.02)] flex-shrink-0">
        <div className="flex items-center space-x-3 bg-slate-50 border border-slate-200 p-2 rounded-2xl transition-all focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 focus-within:bg-white group">
          ...
          <button
            ...
            className={`p-2.5 rounded-xl transition-all transform active:scale-95 flex items-center justify-center ${
              isPreparing || streamingMsg || !input.trim()
  ...
  /**
   * [최종 렌더링]
   * 확대 상태일 때는 Portal을 통해 document.body에 렌더링하고,
   * 평상시에는 원래 위치에 렌더링합니다.
   */
  if (isExpanded) {
