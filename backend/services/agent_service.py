...
class ChatbotAgent:
    def __init__(self):
      # 팩토리를 통해 현재 설정된 제공자(Provider)의 LLM을 주입받음
    ...
    def node_router(self, state: AgentState):
      ...
      # 의도 분류용 모델도 팩토리에서 가져오되, 가벼운 모델을 지정할 수 있습니다.
      # (Vertex AI일 경우 gemini-1.5-flash, OpenAI일 경우 gpt-4o-mini 등)
      base_llm = LLMFactory.get_chat_model(temperature=0, model_name="gemini-1.5-flash" if os.getenv("LLM_PROVIDER") == "vertexai" else "gpt-4o-mini")
